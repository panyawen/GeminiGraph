...
public:
  ...
  void init() {
    ...
    struct bitmask * nodemask = numa_parse_nodestring("all"); //需要注意
    ...
    for (int t_i=0;t_i<threads;t_i++) { //每个线程必须运行在对应的cpu核上，否则无法利用numa的内存特性
  ...
  void load_undirected_from_directed(std::string path, VertexId vertices) {
    ...
    delete [] global_partition_offset;
    {
      ...
      EdgeId remained_amount = part_out_edges + EdgeId(owned_vertices) * alpha; // 8 *（partitions - 1）
      for (int s_i=0;s_i<sockets;s_i++) {
  ...
  // transpose the graph
  void transpose() {
    std::swap(out_degree, in_degree);  //vertex's degree
    std::swap(outgoing_edges, incoming_edges); //the number of edges of each socket
    std::swap(outgoing_adj_index, incoming_adj_index); 
    std::swap(outgoing_adj_bitmap, incoming_adj_bitmap);
    std::swap(outgoing_adj_list, incoming_adj_list); //the edges of each socket
    std::swap(tuned_chunks_dense, tuned_chunks_sparse);
    std::swap(compressed_outgoing_adj_vertices, compressed_incoming_adj_vertices);//the number of each socket's adj vertex
  ...
  void load_directed(std::string path, VertexId vertices) {
    ...
    out_degree = alloc_interleaved_vertex_array<VertexId>();
    // #pragma omp parallel for
    for (VertexId v_i=0;v_i<vertices;v_i++) { //可以考虑并行omp
      out_degree[v_i] = 0;
    }
    assert(lseek(fin, read_offset, SEEK_SET)==read_offset);
    read_bytes = 0;
    while (read_bytes < bytes_to_read) {
      ...
      assert(curr_read_bytes>=0);
      read_bytes += curr_read_bytes;
      EdgeId curr_read_edges = curr_read_bytes / edge_unit_size; //要保证读出来的字节数是edge_unit_size的整倍数，读取错误
      #pragma omp parallel for
      for (EdgeId e_i=0;e_i<curr_read_edges;e_i++) {
        VertexId src = read_edge_buffer[e_i].src; //分布式情况下，机器字节序不一致时，将导致异常（coredump或者运行结果不正确）
        VertexId dst = read_edge_buffer[e_i].dst;
        __sync_fetch_and_add(&out_degree[src], 1); //输入文件应该为二进制文件，否则此处coredump
      }
    }
    MPI_Allreduce(MPI_IN_PLACE, out_degree, vertices, vid_t, MPI_SUM, MPI_COMM_WORLD);
    ...
    for (int i=0;i<partitions;i++) {
      ...
      if (remained_partitions==1) {
        ...
        EdgeId got_edges = 0;
        for (VertexId v_i=partition_offset[i];v_i<vertices;v_i++) {
          got_edges += out_degree[v_i] + alpha;  //此处仅考虑出度，可能造成负载不均衡，可以同时考虑出度和入度
          if (got_edges > expected_chunk_size) {
            partition_offset[i+1] = v_i;
            break;
          }
      ...
      for (VertexId v_i=partition_offset[i];v_i<partition_offset[i+1];v_i++) {
        remained_amount -= out_degree[v_i] + alpha; // 8 *（partitions - 1）
      }
    }
    assert(partition_offset[partitions]==vertices);
    owned_vertices = partition_offset[partition_id+1] - partition_offset[partition_id];
    ...
    {
      std::thread recv_thread_dst([&](){ // 可以换成多线程，降低预处理时间
        int finished_count = 0;
        MPI_Status recv_status;
        while (finished_count < partitions) {
          MPI_Probe(MPI_ANY_SOURCE, ShuffleGraph, MPI_COMM_WORLD, &recv_status);
          ...
          for (EdgeId e_i=0;e_i<recv_edges;e_i++) {
            ...
            __sync_fetch_and_add(&outgoing_adj_index[dst_part][src], 1); //此处可以去掉同步，只有一个线程接收数据
      ...
      assert(lseek(fin, read_offset, SEEK_SET)==read_offset);
      read_bytes = 0;
      while (read_bytes < bytes_to_read) { //此处可以改成多线程发送
        long curr_read_bytes;
        if (bytes_to_read - read_bytes > edge_unit_size * CHUNKSIZE) {
          curr_read_bytes = read(fin, read_edge_buffer, edge_unit_size * CHUNKSIZE);
        } else {
  ...
  R process_edges(std::function<void(VertexId)> sparse_signal, std::function<R(VertexId, M, VertexAdjList<EdgeData>)> sparse_slot, std::function<void(VertexId, VertexAdjList<EdgeData>)> dense_signal, std::function<R(VertexId, M)> dense_slot, Bitmap * active, Bitmap * dense_selective = nullptr) {
    ...
    if (sparse) {
      ...
      std::thread send_thread([&](){ //开启线程进行通信，计算与通信重叠
        std::vector<std::thread> threads;
        for (int step=1;step<partitions;step++) {  //发送给所有的partitons，可能不需要发送给部分partitons
          int i = (partition_id - step + partitions) % partitions; //为什么向前
      ...
      std::thread recv_thread([&](){ 
        ...
        for (int step=1;step<partitions;step++) {
          int i = (partition_id + step) % partitions; //为什么向后
          threads.emplace_back([&](int i){
            for (int s_i=0;s_i<sockets;s_i++) { //其他partition调sockets次MPI_Send，此处就调sockets次MPI_Recv？消息不会合并？若node之间sockets不一致如何？
              MPI_Status recv_status;
      ...
      if (dense_selective!=nullptr && partitions>1) {
        ...
        std::thread send_thread([&](){
          for (int step=1;step<partitions;step++) {
            int recipient_id = (partition_id + step) % partitions; //owned_vertices不一定被64整除
      ...
      std::thread send_thread([&](){
        ...
        for (int step=0;step<partitions;step++) {
          ...
          threads.emplace_back([&](int i){  //多线程发送
            for (int s_i=0;s_i<sockets;s_i++) {
      ...
      std::thread recv_thread([&](){
        ...
        for (int step=1;step<partitions;step++) {
          int i = (partition_id - step + partitions) % partitions;
          threads[step-1].join();  //按顺序join，将导致线头阻塞，其他线程已经收完了数据却不能及时进行处理
          recv_queue[recv_queue_size] = i;
          recv_queue_mutex.lock();
          recv_queue_size += 1;
          recv_queue_mutex.unlock();
